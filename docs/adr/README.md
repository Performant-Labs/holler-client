# ADRs (holler-client)

**Do not open a new GitHub issue for an ADR.** Reuse the reserved slot in [#2](https://github.com/Performant-Labs/holler-client/issues/2)–[#21](https://github.com/Performant-Labs/holler-client/issues/21): retitle, write the brief on the issue, keep the `adr` label.

Issue #1 is orientation. The v1 epic starts at #22.

Circuit-wide decisions (talk plane, tokens, WebSocket, presence, **protocol v1**) live on [holler-server ADRs](https://github.com/Performant-Labs/holler-server/blob/main/docs/adr/README.md). Client ADRs are for this body only.

| ADR | Issue | Title |
|-----|-------|--------|
| 0001 | #2 | Speak Holler protocol v1; answer query locally |
| 0002 | #3 | Bodies are config + ACP subprocess, not plugins |
| 0003–0020 | #4–#21 | reserved |
